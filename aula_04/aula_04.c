...
// gcc -o aula_04 aula_04.c -L/System/Library/Frameworks -framework GLUT -framework OpenGL
...
void Timer(int value) {
    ...
    GLint novoX = modelo.bola.x + modelo.bola.velocidade * cos(modelo.bola.direccao);
    GLint novoY = modelo.bola.y + modelo.bola.velocidade * sin(modelo.bola.direccao);
    ...
    if (novoX >= LARGURA_CAMPO - TAMANHO_BOLA) {
        modelo.bola.x = 200;
        modelo.bola.y = 100;

        modelo.jogador1.pontuacao += 1;

        modelo.bola.direccao = M_PI - modelo.bola.direccao;
    }

    if (novoX <= 0 + TAMANHO_BOLA) {
        modelo.bola.x = 200;
        modelo.bola.y = 100;

        modelo.jogador2.pontuacao += 1;
        modelo.bola.direccao = M_PI - modelo.bola.direccao;
    }

    if (novoY >= ALTURA_CAMPO - TAMANHO_BOLA) {
        modelo.bola.direccao = -1 * modelo.bola.direccao;
    }

    if (novoY <= 0 + TAMANHO_BOLA) {
        modelo.bola.direccao = -1 * modelo.bola.direccao;
    }

    printf("%d", modelo.bola.direccao);

    // testa colisão raquete 1
    if (
        novoX >= modelo.jogador1.x - (LARGURA_RAQUETES / 2) - TAMANHO_BOLA &&
        novoX <= modelo.jogador1.x + (LARGURA_RAQUETES / 2) + TAMANHO_BOLA &&
        novoY >= modelo.jogador1.y - (ALTURA_RAQUETES / 2) - TAMANHO_BOLA &&
        novoY <= modelo.jogador1.y + (ALTURA_RAQUETES / 2) + TAMANHO_BOLA) {
        if (novoX >= modelo.jogador1.x - (LARGURA_RAQUETES / 2) - TAMANHO_BOLA &&
            novoX <= modelo.jogador1.x + (LARGURA_RAQUETES / 2) + TAMANHO_BOLA) {
            modelo.bola.direccao = M_PI - modelo.bola.direccao;
        }
    ...
    if (
        novoX >= modelo.jogador2.x - (LARGURA_RAQUETES / 2) - TAMANHO_BOLA &&
        novoX <= modelo.jogador2.x + (LARGURA_RAQUETES / 2) + TAMANHO_BOLA &&
        novoY >= modelo.jogador2.y - (ALTURA_RAQUETES / 2) - TAMANHO_BOLA &&
        novoY <= modelo.jogador2.y + (ALTURA_RAQUETES / 2) + TAMANHO_BOLA) {
        if (novoX >= modelo.jogador2.x - (LARGURA_RAQUETES / 2) - TAMANHO_BOLA &&
            novoX <= modelo.jogador2.x + (LARGURA_RAQUETES / 2) + TAMANHO_BOLA) {
            modelo.bola.direccao = M_PI - modelo.bola.direccao;
        }

        if (novoY >= modelo.jogador2.y - (ALTURA_RAQUETES / 2) - TAMANHO_BOLA &&
            novoY <= modelo.jogador2.y + (ALTURA_RAQUETES / 2) + TAMANHO_BOLA) {
            modelo.bola.direccao = -1 * modelo.bola.direccao;
        }
    }
